...
from aws_mqaserver.team_Display.models import DisplayLineConfig
...
def update_line_config(request):
    ...
    if id == None:
        try:
            entry = DisplayLineConfig.objects.get(lob=lob, site=site, productLine=productLine, project=project, part=part, domain=domain)
            entry.data = data
            entry.save()
            return response.ResponseData('Add Success')
        except DisplayLineConfig.DoesNotExist:
            entry = DisplayLineConfig(lob=lob, site=site, productLine=productLine, project=project, part=part,
                               domain=domain,
                               data=data)
            entry.save()
            return response.ResponseData('Add Success')
        ...
        entry = DisplayLineConfig.objects.get(id=id)
        entry.data = data
        entry.save()
...
def find_line_config(request):
    ...
    try:
        ...
        try:
            entry = DisplayLineConfig.objects.get(lob=lob, site=site, productLine=productLine, project=project, part=None, domain=domain)
            return response.ResponseData(model_to_dict(entry))
        except DisplayLineConfig.DoesNotExist:
            try:
                entry = DisplayLineConfig.objects.get(lob=lob, site=site, productLine=productLine, project=None, part=None, domain=domain)
                return response.ResponseData(model_to_dict(entry))
            except DisplayLineConfig.DoesNotExist:
                try:
                    entry = DisplayLineConfig.objects.get(lob=lob, site=site, productLine=None, project=None, part=None, domain=domain)
                    return response.ResponseData(model_to_dict(entry))
                except DisplayLineConfig.DoesNotExist:
                    try:
                        entry = DisplayLineConfig.objects.get(lob=lob, site=None, productLine=None, project=None, part=None, domain=domain)
                        return response.ResponseData(model_to_dict(entry))
                    except DisplayLineConfig.DoesNotExist:
                        try:
                            entry = DisplayLineConfig.objects.get(lob=None, site=None, productLine=None, project=None, part=None, domain=domain)
                            return response.ResponseData(model_to_dict(entry))
                        except DisplayLineConfig.DoesNotExist:
...
def get_line_configs_page(request):
    ...
    if site != None:
        if lob == None:
            return response.ResponseError('Params Error')
    try:
        list = DisplayLineConfig.objects.all()
        if lob != None:
