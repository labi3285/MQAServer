...
from aws_mqaserver.team_Display.models import DisplayAuditItemCheckItemEnclosure
...
def _get_audit_item_check_items_page(request, type):
    ...
    try:
        ...
        if type == DisplayCheckType.Enclosure:
            list = DisplayAuditItemCheckItemEnclosure.objects.all().filter(auditItemId=auditItemId).order_by("sn")
        if list is None:
...
def _batch_add_check_items(auditItemId, lob, site, productLine, project, part, type, auditorId, auditor, uploadTime, dicArr):
    ...
    if type == DisplayCheckType.Enclosure:
        for e in dicArr:
            ...
            item = DisplayAuditItemCheckItemEnclosure(
            ...
            batch.append(item)
        if len(batch) > 0:
            DisplayAuditItemCheckItemEnclosure.objects.bulk_create(batch, batch_size=len(batch))
