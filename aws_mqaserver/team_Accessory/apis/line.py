...
from aws_mqaserver.team_Accessory.models import AccessoryLine
...
def add_line(request):
    ...
    try:
        AccessoryLine.objects.get(lob=lob, site=site, productLine=productLine, project=project, part=part)
        return response.ResponseError('Name Duplicate')
    except AccessoryLine.DoesNotExist:
        pass
    except Exception:
        traceback.print_exc()
    ...
    if site == None and operator.role == 'lob_dri':
        ...
        user.save()
    # add line
    line = AccessoryLine(lob=lob, site=site, productLine=productLine, project=project, part=part)
    line.save()
...
def delete_line(request):
    ...
    line = None
    try:
        line = AccessoryLine.objects.get(id=id)
    except AccessoryLine.DoesNotExist:
    ...
    try:
        if line.part != None:
            ...
            if line.project != None:
                list = AccessoryLine.objects.filter(lob=line.lob, site=line.site, productLine=line.productLine, project=line.project)
            elif line.productLine != None:
                list = AccessoryLine.objects.filter(lob=line.lob, site=line.site, productLine=line.productLine)
            elif line.site != None:
                list = AccessoryLine.objects.filter(lob=line.lob, site=line.site)
            elif line.lob != None:
                list = AccessoryLine.objects.filter(lob=line.lob)
            for e in list:
                e.delete()
...
def get_lines_page(request):
    ...
    if site != None:
        if lob == None:
            return response.ResponseError('Params Error') 
    try:
        list = AccessoryLine.objects.all()
        if lob != None:
...
def get_level_lines(request):
    ...
    try:
        list = None
        if lob == None:
            list = AccessoryLine.objects.filter(lob__isnull=False, site__isnull=True)
        else:
            list = AccessoryLine.objects.filter(lob=lob)
            if site == None:
                ...
                for productLine in lobs_dic.get(lob).get(site):
                    ...
                    for project in lobs_dic.get(lob).get(site).get(productLine):
                        ...
                        for e in lobs_dic.get(lob).get(site).get(productLine).get(project):
                            ...
                            checkListId_Glue = value.safe_get_in_key(e, 'checkListId_Glue')
                            checkListId_Destructive = value.safe_get_in_key(e, 'checkListId_Destructive')
                            if project == 'Glue' and checkListId_Glue != None:
                                parts_check_list_uploaded += 1
                            elif project == "Destructive" and checkListId_Destructive != None:
                                parts_check_list_uploaded += 1
                            parts.append({
                                'id': e.get('id'),
                                'lob': lob,
                                ...
                                'name': e.get('part'),
                                'type': 'part',
                                'checkListId_Glue': checkListId_Glue,
                                'checkListId_Destructive': checkListId_Destructive,
