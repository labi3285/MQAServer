...
from aws_mqaserver.team_SIP.models import SIPMILItem
...
def update_mil_item(request):
    ...
    if operator.role == 'admin' and operator.team != 'SIP':
        return response.ResponseError('Operation Forbidden')
    if (operator.role == 'lob_dri' or operator.role == 'lob_auditor') and (operator.team != 'SIP' or not ids.contains_id(lob, operator.lob)):
        return response.ResponseError('Operation Forbidden')
    sn = 0
    ...
    if id == None:
        ...
        entry = SIPMILItem.objects.get(id=id)
        # entry.lob = lob
        # entry.site = site
...
def delete_mil_item(request):
    ...
    try:
        entry = SIPMILItem.objects.get(id=id)
    except SIPMILItem.DoesNotExist:
...
def _batch_add_mil_items(auditItemId, lob, site, productLine, project, part, type, auditorId, auditor, dicArr):
    ...
    for e in dicArr:
        ...
        batch.append(item)
    if len(batch) > 0:
        SIPMILItem.objects.bulk_create(batch, batch_size=len(batch))
