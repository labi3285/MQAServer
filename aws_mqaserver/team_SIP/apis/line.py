...
def add_line(request):
    ...
    try:
        SIPLine.objects.get(lob=lob, site=site, productLine=productLine, project=project, part=part)
        return response.ResponseError('Name Duplicate')
    except SIPLine.DoesNotExist:
        pass
    except Exception:
        traceback.print_exc()
    ...
    if site == None and operator.role == 'lob_dri':
        ...
        user.save()
    # add line
    line = SIPLine(lob=lob, site=site, productLine=productLine, project=project, part=part)
    line.save()
...
def delete_line(request):
    ...
    line = None
    try:
        line = SIPLine.objects.get(id=id)
    except SIPLine.DoesNotExist:
    ...
    try:
        if line.part != None:
            ...
            if line.project != None:
                list = SIPLine.objects.filter(lob=line.lob, site=line.site, productLine=line.productLine, project=line.project)
            elif line.productLine != None:
                list = SIPLine.objects.filter(lob=line.lob, site=line.site, productLine=line.productLine)
            elif line.site != None:
                list = SIPLine.objects.filter(lob=line.lob, site=line.site)
            elif line.lob != None:
                list = SIPLine.objects.filter(lob=line.lob)
            for e in list:
                e.delete()
...
def get_level_lines(request):
    ...
    try:
        list = None
        if lob == None:
            list = SIPLine.objects.filter(lob__isnull=False, site__isnull=True)
        else:
            list = SIPLine.objects.filter(lob=lob)
            if site == None:
